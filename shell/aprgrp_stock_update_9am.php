...
function settleUpStock($productId, $isConfigProduct = false, $qty = 0)
{
    ...
    if ($isConfigProduct === false)//don't set quantity for configurable products
...
//First of all empty all the stock
logMsg("Emptying stock...\n");
$sellerPrds = Mage::getSingleton('catalog/product')
    ->getCollection()
    ->addAttributeToFilter('seller_id',array('in', [294, 293, 292, 291]));

$emptied = false;
foreach($sellerPrds as $prd) {
    $stockItem = Mage::getSingleton('cataloginventory/stock_item')->loadByProduct($prd->getId());
    $stockItemId = $stockItem->getId();
    $stockItem->setData('manage_stock', 1);
...
if($emptied) {
    logMsg("Stock emptied!\n");
} else {
    logMsg("ERROR >>> Stock couldn't be emptied!\n");
}
...
foreach ($csvDataArray as $row => $rdata) {
    try {
        ...
        foreach ($products as $product) {
            ...
            if ($priceProduct->getPrice() != $rdata[2]) {//only update the price if the value is different
