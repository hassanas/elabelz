...
function logMsg($msg, $display = true)
{
    ...
    Mage::log("apprg_sales_report:".$msg);
}
...
foreach($orderCollection as $order) {
    ...
    $sellerId = Mage::getModel('marketplace/commission')->load($order->getIncrementId(), 'increment_id')->getSellerId();
    //$storeName = $sellerProfile->getStoreTitle();
    //$sellerProfile = Mage::getModel('marketplace/sellerprofile')->load($sellerId, 'seller_id');
    //$storeName = $sellerProfile->getStoreTitle();
...
try {
    if(!empty($data) && $csv->saveData($file, $data)) {
        ...
        if(empty($data)) logMsg("because there is no sale record of last one day for this seller.\n");
    }
} catch (Exception $e) {
