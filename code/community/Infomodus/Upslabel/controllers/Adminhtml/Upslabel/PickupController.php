...
class Infomodus_Upslabel_Adminhtml_Upslabel_PickupController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_addBreadcrumb(Mage::helper('upslabel')->__('Pickup Manager'), Mage::helper('upslabel')->__('Pickup Manager'));
    ...
    public function editAction()
    {
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->_addBreadcrumb(Mage::helper('upslabel')->__('Pickup Manager'), Mage::helper('upslabel')->__('Pickup Manager'));
            $this->_addBreadcrumb(Mage::helper('upslabel')->__('Pickup News'), Mage::helper('upslabel')->__('Pickup News'));
        ...
        if ($data = $this->getRequest()->getPost()) {
            try {
                ...
                if (!isset($response['error'])) {
                    ...
                    $this->getResponse()->setBody($response['error']);
                    return;
                }
            } catch (Exception $e) {
    ...
    public function deleteAction()
    {
        if ($this->getRequest()->getParam('id') > 0) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('upslabel')->__('Pickup was successfully deleted'));
                $this->_redirect('*/*/');
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
                $this->_redirect('*/*/edit', array('id' => $this->getRequest()->getParam('id')));
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($upslabelIds)) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('upslabel')->__('Please select Pickup(s)'));
        } else {
            try {
