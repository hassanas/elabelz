 ...
 * @package BoxPacker
 */
class Infomodus_Dhllabel_Model_Packer_VolumePacker
{
     ...
     * Constructor
     */
    public function __construct(Infomodus_Dhllabel_Model_Packer_Box $box, Infomodus_Dhllabel_Model_Packer_ItemList $items)
    {
    ...
    public function pack()
    {
        ...
        $packedItems = new Infomodus_Dhllabel_Model_Packer_ItemList;
        ...
        return new Infomodus_Dhllabel_Model_Packer_PackedBox($this->box, $packedItems, $this->widthLeft, $this->lengthLeft, $this->depthLeft, $this->remainingWeight);
    }
     ...
     */
    protected function findBestOrientation(Infomodus_Dhllabel_Model_Packer_Item $item, Infomodus_Dhllabel_Model_Packer_OrientatedItem $prevItem = null, Infomodus_Dhllabel_Model_Packer_Item $nextItem = null, $widthLeft, $lengthLeft, $depthLeft) {
     ...
     */
    protected function findPossibleOrientations(Infomodus_Dhllabel_Model_Packer_Item $item, Infomodus_Dhllabel_Model_Packer_OrientatedItem $prevItem = null, $widthLeft, $lengthLeft, $depthLeft) {

        $orientations = array();

        //Special case items that are the same as what we just packed - keep orientation
        if ($prevItem && $prevItem->getItem() == $item) {
            $orientations[] = new Infomodus_Dhllabel_Model_Packer_OrientatedItem($item, $prevItem->getWidth(), $prevItem->getLength(), $prevItem->getDepth());
        } else {

            //simple 2D rotation
            $orientations[] = new Infomodus_Dhllabel_Model_Packer_OrientatedItem($item, $item->getWidth(), $item->getLength(), $item->getDepth());
            $orientations[] = new Infomodus_Dhllabel_Model_Packer_OrientatedItem($item, $item->getLength(), $item->getWidth(), $item->getDepth());

            //add 3D rotation if we're allowed
            if (!$item->getKeepFlat()) {
                $orientations[] = new Infomodus_Dhllabel_Model_Packer_OrientatedItem($item, $item->getWidth(), $item->getDepth(), $item->getLength());
                $orientations[] = new Infomodus_Dhllabel_Model_Packer_OrientatedItem($item, $item->getLength(), $item->getDepth(), $item->getWidth());
                $orientations[] = new Infomodus_Dhllabel_Model_Packer_OrientatedItem($item, $item->getDepth(), $item->getWidth(), $item->getLength());
                $orientations[] = new Infomodus_Dhllabel_Model_Packer_OrientatedItem($item, $item->getDepth(), $item->getLength(), $item->getWidth());
            }
        }

        //remove any that simply don't fit
        return array_filter($orientations, function (Infomodus_Dhllabel_Model_Packer_OrientatedItem $i) use ($widthLeft, $lengthLeft, $depthLeft) {
     ...
     * @param int $maxDepth
     */
    protected function tryAndStackItemsIntoSpace(Infomodus_Dhllabel_Model_Packer_ItemList $packedItems, $maxWidth, $maxLength, $maxDepth)
    {
