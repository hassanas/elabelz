...
class MSP_CashOnDelivery_Model_Quote_Tax extends Mage_Sales_Model_Quote_Address_Total_Tax
{
    ...
    public function collect(Mage_Sales_Model_Quote_Address $address)
    {
        ...
        $baseAmount = $_model->getBaseExtraFee($quote, $_subtotal);
        $amount = $_model->getExtraFee($quote);

        $codTax = $_helper->getTaxAmount($quote, $amount);
        $codBaseTax = $_helper->getTaxAmount($quote, $baseAmount);
        ...
        if (
            ...
            $this->_saveAppliedTaxes(
                $address,
                $_helper->getAppliedRates($quote),
                $codTax,
                $codBaseTax,
                $_helper->getRate($quote)
            );
