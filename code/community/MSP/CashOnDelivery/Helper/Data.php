...
class MSP_CashOnDelivery_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getZoneType(Mage_Sales_Model_Quote $quote)
    {
    ...
    public function getStandardFixedFee(Mage_Sales_Model_Quote $quote, $storeId = null)
    {
        if ($this->getZoneType($quote) == self::MSP_COD_LOCAL) {
    ...
    public function getRate(Mage_Sales_Model_Quote $quote)
    {
        if (true || !$this->_rate) {
            ...
            $request = $taxCalculationModel->getRateRequest($address, $address->getQuote()->getBillingAddress(),
    ...
    public function getAppliedRates(Mage_Sales_Model_Quote $quote)
    {
        $address = $quote->getShippingAddress();
        $store = $address->getQuote()->getStore();
        $custTaxClassId = $address->getQuote()->getCustomerTaxClassId();
    ...
    public function excludeTax(Mage_Sales_Model_Quote $quote, $amount)
    {
        $rate = $this->getRate($quote);
        return $amount / (1 + $rate / 100);
    }

    public function includeTax(Mage_Sales_Model_Quote $quote, $amount)
    {
        $rate = $this->getRate($quote);
        return $amount * (1 + $rate / 100);
    }

    public function getTaxAmount(Mage_Sales_Model_Quote $quote, $amountExclTax)
    {
        $rate = $this->getRate($quote);
