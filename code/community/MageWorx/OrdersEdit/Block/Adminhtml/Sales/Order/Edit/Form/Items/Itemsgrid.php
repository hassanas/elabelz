...
class MageWorx_OrdersEdit_Block_Adminhtml_Sales_Order_Edit_Form_Items_Itemsgrid extends Mage_Adminhtml_Block_Sales_Order_Create_Items_Grid
{
    ...
    public function getConfigureButtonHtml($item)
    {
        ...
        $_product = Mage::getModel('catalog/product')
                ->getCollection()
                ->addAttributeToFilter('sku',$productSku)->getFirstItem();
        ...
        if ($product->canConfigure()) {
            if($marketplace->getId() != ""){
               $options['onclick'] = "javascript: alert('Please dont change the size or color of any product, this will not show in the sellers dashboard');";
            }else{
