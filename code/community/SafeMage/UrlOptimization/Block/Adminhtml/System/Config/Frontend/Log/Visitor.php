...
class SafeMage_UrlOptimization_Block_Adminhtml_System_Config_Frontend_Log_Visitor extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    ...
    protected function _getAddTotalCountHtml() {
        ...
        $select = $connection->select()->from($resource->getTableName('log/visitor'), array('COUNT(*)'));
        ...
        $select = $connection->select()->from($resource->getTableName('log/visitor'), array('MIN(last_visit_at)'));
