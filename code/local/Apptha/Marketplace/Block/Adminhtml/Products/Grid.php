...
class Apptha_Marketplace_Block_Adminhtml_Products_Grid extends Mage_Adminhtml_Block_Widget_Grid {
 ...
 protected function _prepareCollection() {
  ...
  /*------------By Azhar---------------*/
   $collection->addAttributeToSelect ( 'seller_product_status' );
  ...
  $collection->addAttributeToSort('id', 'asc');
 ...
 protected function _prepareColumns() {
  ...
  if (Mage::helper ( 'catalog' )->isModuleEnabled ( 'Mage_CatalogInventory' )) {
   $this->addColumn ( 'qty', array (
     ...
     if (Mage::helper ( 'catalog' )->isModuleEnabled ( 'Mage_CatalogInventory' )) {
         $this->addColumn ( 'qty_hold', array (
             'header' => Mage::helper ( 'catalog' )->__ ( 'Qty in Hold' ),
             'width' => '50px',
             'type' => 'number',
             'productid'=>'entity_id',
             'renderer' => 'Apptha_Marketplace_Block_Adminhtml_Renderersource_HoldItems'
         ) );
     }

     if (Mage::helper ( 'catalog' )->isModuleEnabled ( 'Mage_CatalogInventory' )) {
         $this->addColumn ( 'qty_total', array (
             'header' => Mage::helper ( 'catalog' )->__ ( 'Total Physical Qty' ),
             'width' => '50px',
             'type' => 'number',
             'productid'=>'entity_id',
             'magentoqty'=>'qty',
             'renderer' => 'Apptha_Marketplace_Block_Adminhtml_Renderersource_TotalItems'
         ) );
     }
  $this->addColumn ( 'status', array (
   ...
   $this->addColumn('seller_product_status', array(
            'header'    =>Mage::helper('reports')->__('Seller product status'),
            'sortable'  =>false,
            'index'     =>'seller_product_status',
            'type'  => 'options',
            'options'=> Mage::getSingleton('catalog/product_featured')->sellerproductstatus()
        ));
    $this->addColumn('featured',
      array(
        ...
        'default'   =>    '----',
        'type'  => 'options',
        'options' => Mage::getSingleton('catalog/product_featured')->getOptionArray()
    ));
    ...
    'filter' => false,
    'sortable' => false,
    'renderer' => 'Apptha_Marketplace_Block_Adminhtml_Renderersource_AttributeValue' 
  ) );
