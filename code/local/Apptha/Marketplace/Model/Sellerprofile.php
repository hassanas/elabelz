...
class Apptha_Marketplace_Model_Sellerprofile extends Mage_Core_Model_Abstract {
    ...
    /**
     * Set values of seller store titles, the function will be used by cronjob to set the store titles
     */
    function setSellerStoreTitles()
    {
        try {
            $customerArr = array();
            $customerArr[] = $this->getAllOptions();
            $tittle_array = array(0,"null","");
            ...
            foreach ($collection as $product_new) {
                ...
                if(!$products->getData('seller_store_name_attr')):
                    //echo "<br/>".$product_new->getId()."<br/>";
                    $products = Mage::getModel('catalog/product')->load($product_new->getId());
                    //echo $products->getData('seller_store_name_attr')."<br/>";
                    // echo $product_new->getSellerId();
                    // $products = Mage::getModel('catalog/product')->load($product_new->getId());
                    $product_new->setData('seller_store_name_attr',$product_new->getSellerId());
                    $product_new->save();
                endif;
            }
            //mail("humaira.batool@progos.org","data passed for store tittle","done");
        } catch (Exception $e) {
            logException($e);
        }
    }
