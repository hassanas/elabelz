...
class Apptha_Marketplace_Model_Seller extends Mage_Core_Model_Abstract {
     ...
     * @return $collection
     */
    public function setSellerProfileDetails($collection, $data, $storeLogo, $storeBanner, $imagesPathLogo, $imagesPathBanner , $storeFile = '' , $samplePath = '' ) {
        /**
         * check remove logo has been posted
         */ 
        $removeLogo = $removeBanner = $showProfile = $removeSampleFile = '';
		...
		/**
         * check remove Sample File has been posted
         */
        if (isset ( $data ['remove_sample_product_profile_file'] )) {
            $removeSampleFile = $data ['remove_sample_product_profile_file'];
        }
		...
		/**
         * check empty or not of $storeFile
         */
        if (! empty ( $storeFile )) {
            $collection->setSampleProductProfileFile ( $samplePath );
        }
		
		//Directory Path
		$directory = Mage::getBaseDir('media') . DS .'sellerimage'. DS;
        /**
         * remove logo
         */
        if ($removeLogo == 1) {
            $collection->setStoreLogo ( '' );
			if (file_exists( $directory .$data['sl_hidden'] ))
				unlink( $directory .$data['sl_hidden']);
        }
        /**
         * remove banner
         */
        if ($removeBanner == 1) {
			...
			if (file_exists( $directory .$data['sb_hidden']))
				unlink( $directory .$data['sb_hidden']);
        }
		/**
         * remove File
         */
        if ($removeSampleFile == 1) {
            $collection->setSampleProductProfileFile ( '' );
			if (file_exists( $directory .$data['sf_hidden'] ))
				unlink( $directory .$data['sf_hidden']);
        }
     ...
     *
     */
    public function saveSellerProfileData($sellerId, $imageInfoForSeller, $showProfile, $data, $storeLogo, $storeBanner, $flag , $storeFile = '' ) {  
    	/**
		...
		//Directory Path
		$directory = Mage::getBaseDir('media') . DS .'sellerimage'. DS;
		/* Remove Previous banner if seller selected new one. */
		if( !empty($imageInfoForSeller['images_path_banner']) ){
			if( $imageInfoForSeller['images_path_banner'] != $data['sb_hidden'] && file_exists( $directory .$data['sb_hidden']) )
				unlink( $directory .$data['sb_hidden']);
		}
		/* Remove Previous Logo if seller selected new one. */
		if( !empty($imageInfoForSeller['images_path_logo']) ){
			if( $imageInfoForSeller['images_path_logo'] != $data['sl_hidden'] && file_exists( $directory .$data['sl_hidden'] ) )
				unlink( $directory .$data['sl_hidden']);
		}
		/* Remove Previous File if seller selected new one. */
		if( !empty($imageInfoForSeller['product_profile_path']) ){
			if( $imageInfoForSeller['product_profile_path'] != $data['sf_hidden'] && file_exists( $directory .$data['sf_hidden'] ) )
				unlink( $directory .$data['sf_hidden']);
		}
		...
		$imagesSampleProduct = $imageInfoForSeller ['product_profile_path'];
        $state = $data ['state'];
		...
		$have_store = $data['have_store']; 
		$no_of_style = $data['no_of_style']; 		
        
        if ($flag == 1) {
            $collection = Mage::getModel ( 'marketplace/sellerprofile' )->load ( $sellerId, 'seller_id' );
             
             
            $collection = Mage::getModel ( 'marketplace/seller' )->setSellerProfileDetails ( $collection, $data, $storeLogo, $storeBanner, $imagesPathLogo, $imagesPathBanner ,$storeFile , $imagesSampleProduct );
        } else {
       ...
       */
        ...
        $collection->setSupplierAddress($supplier_address);
		$collection->setCategoryProduct( $category_product );
		$collection->setHaveStore( $have_store );
		$collection->setNoOfStyle( $no_of_style );
        ...
        if ($flag != 1) {
			$collection->setSampleProductProfileFile ( $imagesSampleProduct );
            $collection->setStoreLogo ( $imagesPathLogo );
            $collection->setStoreBanner ( $imagesPathBanner );           
        }
        $collection->save ();
	...
	/**
     * Save seller profile data During Signup Process
     */
	
    public function saveSellerProfileDataSignup($sellerId, $imageInfoForSeller, $showProfile, $data, $storeLogo, $storeBanner, $flag , $sampeFile) {  
    	/**
    	 * Getting seller profile datas
    	 */
        $storeName = $imageInfoForSeller ['store_name'];
       ...
       */
		...
		$collection->setCategoryProduct( $data['category_product'] );
		$collection->setHaveStore( $data['have_store'] );
		$collection->setNoOfStyle( $data['no_of_style'] );
		$collection->setContact ( $data['phone_no'] );
        $collection->setSellerId( $sellerId );
        $collection->setStoreTitle ( $storeName );     
		$collection->setShowProfile ( 0 );
		$collection->setStoreLogo ( $imagesPathLogo );
		$collection->setSampleProductProfileFile ( $imagesPathBanner );           
        $collection->save ();
    }
	...
	/**
     * save Sample File for seller
     *
     * @param array $filesDataArray            
     * @param string $basedir            
     * @return string $filename
     */
    public function saveSampleFileForSeller($filesDataArray, $basedir, $logo) {
        /**
         * check store logo has not been empty
         */
        $filename = '';
