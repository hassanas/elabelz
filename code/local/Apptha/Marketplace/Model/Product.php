...
class Apptha_Marketplace_Model_Product extends Mage_Core_Model_Abstract {
     ...
     * @param array $product            
     */
    public function setBaseImageForProduct($productId, $store, $setbase, $productData, $action,$setThumb,$setSmall) {
        /**
         * Load the product
         */
        ...
        if (isset ( $mediaGallery ['images'] ) && ! empty ( $store )) {
            ...
            foreach ( $mediaGallery ['images'] as $image ) {
                ...
                if ($increment == $setbase) {
                    $productImage->setStoreId ( $store )->setImage ( $image ['file'] );

                }
                elseif($increment == $setThumb){
                    $productImage->setStoreId ( $store )->setThumbnail ( $image ['file'] );
                }
                elseif($increment == $setSmall){
                    $productImage->setStoreId ( $store )->setSmallImage ( $image ['file'] );
                }
                $productImage->save ();
                $increment ++;
                /**
    ...
    public function checkWhetherSupplierSkuExistOrNot($productData) {
        if (isset ( $productData ['supplier_sku'] )) {
            $value = 0;
            $skuProductId = Mage::getModel ( 'catalog/product' )->getIdBySku ( trim ( $productData ['supplier_sku'] ) );
        }
        if (! empty ( $skuProductId )) {
            $value = 1;
        }
     ...
     * @param array $product            
     */
    public function setProductImagesforProduct($baseimage, $productId, $store, $product, $productData, $smallImage,$thumbImage) {
        /**
        ...
        if (! empty ( $productId )) {
            ...
            if(! empty ( $baseimage )){
                $product->setStoreId ( $store )->setImage ( $baseimage );
            }
            else
                $setbase = 0;
            if(! empty ( $smallImage )){
                $product->setStoreId ( $store )->setSmallImage ( $smallImage );
            }
            else
                $setSmall = 0;
            if(! empty ( $thumbImage )){
                $product->setStoreId ( $store )->setThumbnail ( $thumbImage );
            }
            else
                $setThumb = 0;
            $product->save ();

            if(isset($setbase)||isset($setThumb)||isset($setSmall)){
                $this->setBaseImageForProduct ( $productId, $store, $setbase, $productData, 'edit',$thumbImage, $smallImage );
            }
        }
     ...
     * @param number $productId            
     */
    public function deleteProductImagesForEdit($deleteimages, $productId, $baseimage, $product) {
        /**
