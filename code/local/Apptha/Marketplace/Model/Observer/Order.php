...
class Apptha_Marketplace_Model_Observer_Order {
    ...
    public function salesOrderAfter() {
        ...
        if ($orderId) {
            ...
            /*-----------------Adding Failed Delivery Value--------------------------*/
             if($orders->getFailedDelivery() == 0):
                   Mage::helper('progos_ordersedit')->getFailedDeliveryStatus($orders);
            endif;
            /*----------------------------------------------------------------------*/
            ...
            if ($count > 0) {
                ...
                foreach ( $items as $item ) {                	
                    ...
                    if($products_new->getTypeId() == "configurable")
                        {
                           ...
                           $getProductIdValue = Mage::getModel('catalog/product')->getIdBySku($sku);
                       }
                    else{
                        $getProductIdValue = $item->getProductId();
                    }
                    ...
                    $credited = 1;
                    $orderPrice = $item->getBaseOriginalPrice() * $item->getQtyOrdered ();
                    $productAmt = $item->getBaseOriginalPrice();
                    ...
                    $parentIds = Mage::getModel('catalog/product_type_configurable')->getParentIdsByChild($getProductIdValue);
                    ...
                    if ($parent_product->getSpecialPrice()) {
                        ...
                        $orderPrice_base = $item->getBaseOriginalPrice() * $item->getQtyOrdered();
    ...
    public function creditMemoEvent(Varien_Event_Observer $observer) {
        ...
        foreach ( $items as $item ) { 
            ...
            ->addFieldToFilter ( 'product_id', $getProductIdValue )
            ->addFieldToSelect ( 'id' )
            ...
            foreach ( $commissions as $commission ) {
                ...
                $orderPrice = $product->getPrice () * $qty;
                /**
                 ...
                 */
                $shippingPrice = Mage::helper('marketplace/market')->getShippingPrice($sellerDefaultCountry, $shippingCountryId, $orderPrice, $nationalShippingPrice, $internationalShippingPrice, $qty);
                if ($product->getSpecialPrice()) {
                        $orderPrice_sp = $product->getSpecialPrice() * $qty;
                        $orderPrice_base = $product->getPrice() * $qty;
     ...
     * @return void
     */
    public function cancelOrderEmailOld($observer) {
    ...
    public function cancelOrderEmail($observer) {
        ...
        $order = Mage::getModel ( 'sales/order' )->load ( $orderIds );
         ...
         $rejected_customer = Mage::helper('marketplace/vieworder')->getProductConfirmRejected($orderIds);
         if($rejected_customer){
            Mage::helper('marketplace/vieworder')->failedOrderItemEmail($orderIds);
         }
    }
    ...
    public function sendCancelOrderEmail($orderEmailData) {
        ...
        $displayProductCommission = Mage::helper ( 'marketplace' )->__ ( 'SellerCommission Fee' );
    ...
    public function invoiceSaveAfter($observer){
        ...
        $rejected_items = Mage::helper('marketplace/vieworder')->allRejectedSellerProductIds($order->getId());
    ...
    public function checkConfirmedItems()
    {
        $orderId = ( int ) Mage::app ()->getRequest ()->getParam ( 'order_id' );
        if ($orderId) {
