...
class Apptha_Marketplace_Helper_Url extends Mage_Core_Helper_Abstract {
    ...
    public function getManageOrderUrl() {
        ...
        return $ordersUrl."?status=pending";
    }
    ...
    /**
     * Function to get payout/withdrawal request url
     *
     * This Function will return the redirect url to make the payout request
     *
     * @return string
     */
    public function getPayoutUrl() {
        return Mage::getUrl ( 'marketplace/seller/payout' );
    }
    
    
    /**
     * Added by Ali Nasrullah
     * Function to get Transaction/Detail request url
     *
     * This Function will return the redirect url to show the Payout Transaction History
     *
     * @return string
     */
    public function getTransactionDetailUrl() {
        return Mage::getUrl ( 'marketplace/seller/transactiondetail' );
    }

    public function getFareyeNewProcessUrl($prod=true)
    {
        return ($prod === false) ? $this->getFareyeStagingUrl()."process?api_key=".$this->getFareyeApiKey()
            : $this->getFareyeStagingUrl()."process?api_key=".$this->getFareyeApiKey();
    }

    public function getFareyeAddSourceUrl($prod=true)
    {
        return ($prod === false) ? $this->getFareyeStagingUrl()."data_stores?api_key=".$this->getFareyeApiKey()
            : $this->getFareyeStagingUrl()."data_stores?api_key=".$this->getFareyeApiKey();
    }

    public function getFareyeUpdateTransUrl($prod=true)
    {
        return ($prod === false) ? $this->getFareyeStagingUrl()."update_transactions_status?api_key=".$this->getFareyeApiKey()
            : $this->getFareyeStagingUrl()."update_transactions_status?api_key=".$this->getFareyeApiKey();
    }

    public function getFareyeApiKey()
    {
        return "qFUeaPzvoUU9pmSdiTIBfKhkvz3WoAKu";
    }

    public function getFareyeStagingUrl()
    {
        return "https://staging.fareye.co/api/v1/";
    }

    public function getFareyeProdUrl()
    {
        return "https://prod.fareye.co/api/v1/";
    }
}
