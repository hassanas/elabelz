...
class Apptha_Marketplace_Adminhtml_ManagesellerController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function exportCsvAction()
    {
        ...
        $content = $this->getLayout()->createBlock('apptha_marketplace_block_adminhtml_manageseller_grid');
    ...
    public function getErrorMessage()
    {
        Mage::getSingleton('adminhtml/session')
            ->addError(
                Mage::helper('marketplace')->__('Seller details not updated')
            );
    }
    ...
    public function setpayoutstatusAction()
    {
        $this->_initAction();
    ...
    public function successMessage()
    {
        Mage::getSingleton('adminhtml/session')
            ->addSuccess(
                Mage::helper('adminhtml')->__('Seller successfully deleted')
            );
    }
    ...
    function saveprofileAction()
    {
        ...
        if ($this->getRequest()->getParam('id') > 0) {
            ...
            try {
                ...
                /*---------Delete Image------------*/
                if (isset($data['store_banner']['delete']) && $data ['store_banner']['delete'] == 1) {
                    unlink(Mage::getBaseDir('media') . DS . 'sellerimage' . DS . $collection->getStoreBanner());
                    $collection->setStoreBanner("");
                } else {
                    /*If banner is not deleted and banner path is not exist then it don't save record.*/
                    if (!empty($imageInfoForSeller ['images_path_banner'])) {
                        $collection->setStoreBanner($imageInfoForSeller ['images_path_banner']);
                        if ($imageInfoForSeller ['images_path_banner'] != $data['sb_hidden'])
                            unlink(Mage::getBaseDir('media') . DS . 'sellerimage' . DS . $data['sb_hidden']);
                    }
                }

                /*Delete file*/
                if (isset($data['sample_product_profile_file']['delete']) && $data ['sample_product_profile_file']['delete'] == 1) {
                    unlink(Mage::getBaseDir('media') . DS . 'sellerimage' . DS . $collection->getSampleProductProfileFile());
                    $collection->setSampleProductProfileFile("");
                } else {
                    /*If file is not deleted and product path is not exist then it don't save record.*/
                    if (!empty($imageInfoForSeller ['product_profile_path'])) {
                        $collection->setSampleProductProfileFile($imageInfoForSeller ['product_profile_path']);
                        if ($imageInfoForSeller ['product_profile_path'] != $data['sp_hidden'])
                            unlink(Mage::getBaseDir('media') . DS . 'sellerimage' . DS . $data['sp_hidden']);
                    }
                }

                $collection->setStore_title($storeName);
                $collection->setState($data['state_name']);
                $collection->setCountry($data['country']);
                $collection->setContact($data['contact']);
                $collection->setDescription($data['description']);
                $collection->setSupplier_address($data['supplier_address']);
                $collection->setBank_payment($data['bank_payment']);
                $collection->setSellerId($sellerId);
                $collection->setCategoryProduct($data['category_product']);
                if (!empty($data['have_store']))
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('marketplace')->__('Seller profile information has been successfully saved .'));

                $this->_redirect('*/adminhtml_manageseller/index/');
                 ...
                 */
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
                $this->_redirect('*/adminhtml_sellerreview/id/' . $sellerId . '/');
            }
