...
class Apptha_CustomMenu_Block_Navigation extends Mage_Catalog_Block_Navigation {
    ...
    public function drawCustomMenuMobileItem($category, $level = 0) {
         ...
         */
        $html [] = '<a href="' . $catUrl . '">';
        $name = $this->__ ($this->escapeHtml ( $category->getName () ));
        if (Mage::getStoreConfig ( 'custom_menu/general/non_breaking_space' )) {
            $name = str_replace ( ' ', '&nbsp;', $name );
        }
        $html [] = '<span>' . $this->__ ($name) . '</span>';
        $html [] = '</a>';
        /**
    ...
    public function drawCustomMenuItem($category, $level = 0) {
        ...
        $htmlTop [] = '<span>' . $this->__ ($name) . '</span>';
        $htmlTop [] = '</a>';
        $htmlTop [] = '</div>';
        $htmlTop [] = '</div>';
        ...
        if ($drawPopup) {
             ...
             */
            $htmlPopup [] = '<div id="popup' . $id . '" class="custom-menu-popup" onmouseout="HideMenuPopup(this, event, \'popup' . $id . '\', \'menu' . $id . '\')" onmouseover="PopupOver(this, event, \'popup' . $id . '\', \'menu' . $id . '\')"><div class="customPopupHelper">';
    ...
    public function drawMobileMenuItem($children, $level = 1) {
        ...
        foreach ( $children as $child ) {
            ...
            if (is_object ( $child ) && $child->getIsActive ()) {
                 ...
                 */
                $name = $this->__ ($this->escapeHtml ( $child->getName () ));
                if (Mage::getStoreConfig ( 'custom_menu/general/non_breaking_space' )) {
                    $name = str_replace ( ' ', '&nbsp;', $name );
                }
                $html .= '<div class="parentMenu">';
                $html .= '<a class="itemMenuName level' . $level . '" href="' . $this->getCategoryUrl ( $child ) . '"><span>' . $this->__ ($name) . '</span></a>';
                /**
    ...
    public function drawMenuItem($children, $level = 1) {
        ...
        foreach ( $children as $child ) {
            ...
            if (is_object ( $child ) && $child->getIsActive ()) {
                ...
                $html .= '<a class="itemMenuName level' . $level . $active . '" href="' . $this->getCategoryUrl ( $child ) . '"><span>' . $this->__ ($name) . '</span></a>';
                $activeChildren = $this->_getActiveChildren ( $child, $level );
                /**
                 * check condition active children count is greaterthan 0
                 */
