 ...
 */
require_once(Mage::getModuleDir('lib', 'Apptha_Sociallogin') . '/lib/Facebook/autoload.php');
...
class Apptha_Sociallogin_IndexController extends Mage_Core_Controller_Front_Action
{
     ...
     * @return string
     */
    public function customerAction($firstname, $lastname, $email,$gender=null, $provider, $data)
    {
        ...
        $sellerGroup = Mage::getSingleton('core/session')->getSellerGroup();
        ...
        $standardInfo ['gender'] = $gender;
        if ($gender=='male') {
            $standardInfo ['gender'] = 1;
        } elseif ($gender=='female') {
            $standardInfo ['gender'] = 2;
        }
        ...
        if ($customer->getId()) {
            ...
            } elseif ($loggedCustomerStatus==0 && $customerGroupId != $groupId ) {
                if (Mage::getStoreConfig('sociallogin/general/auto_approve')) {
            ...
            $requestPath=false;
            if (!empty($link)) {
                $requestPath = strpos($link,'onestepcheckout');
            }
            /**
         ...
         */
        if ($adminApproval == 1 && $sellerGroup == 1) {
            $customer->setGroupId($groupId);
            $customer->setCustomerstatus('0');
        } elseif ($adminApproval != 1 && $sellerGroup == 1) {
            $customer->setGroupId($groupId);
            $customer->setCustomerstatus('1');
        } else {
            $customer->setCustomerstatus('1');
    ...
    public function twitterloginAction()
    {
        ...
        if (isset ($userInfo->error)) {
            ...
            if ($email == '' || $firstName == '') {
                ...
                $this->customerAction($firstName, $lastName, $email, $gender,'Twitter', $data);
            }
        }
    }
    ...
    {
        $app_id = Mage::getStoreConfig('sociallogin/facebook/fbapp_id');
        $app_secret = Mage::getStoreConfig('sociallogin/facebook/fbapp_secret');
        $fb = new Facebook\Facebook([
            'app_id' => $app_id,
            'app_secret' => $app_secret,
            'default_graph_version' => 'v2.2',
        ]);
        $fblogin=$this->getLayout()->createBlock('sociallogin/buttons');

        $redirectUri=$fblogin->getFbLoginUrl();
        $session =$this->_getCustomerSession();
        ...
        try {
            ...
            $session->addError('Graph returned an error: ' . $e->getMessage());
            $this->_redirect('customer/account/index');
        } catch (Facebook\Exceptions\FacebookSDKException $e) {
            // When validation fails or other local issues
            $session->addError('Facebook SDK returned an error: ' . $e->getMessage());
            $this->_redirect('customer/account/index');
        }

        if (!isset($accessToken)) {
            if ($helper->getError()) {
                $errormsg = "Error: " . $helper->getError() . "\n";
                $errormsg .= "Error Code: " . $helper->getErrorCode() . "\n";
                $errormsg .= "Error Reason: " . $helper->getErrorReason() . "\n";
                $errormsg .= "Error Description: " . $helper->getErrorDescription() . "\n";
            } else {
...
//$tokenMetadata->validateUserId('123');
        ...
        if (!$accessToken->isLongLived()) {
            ...
            try {
                ...
                $session->addError("Error getting long-lived access token: " . $helper->getMessage());
                $this->_redirect('customer/account/index');
            }
        ...
        if ($email) {
            ...
            $data = $this->getRequest()->getParam('fb');
            $this->customerAction($firstName, $lastName, $email,$gender, 'Facebook', $data);
        } else {
    ...
    public function googlepostAction()
    {
        ...
        if (isset ($authUrl)) {
            ...
            Mage::log($user,null,'sociallogin.log');
            $email = $user ['email'];
            $user ['id'];
            $gender =$user['gender'];
            if ($email == '') {
                ...
                $this->customerAction($firstName, $lastName, $email,$gender, 'Google', $data);
            }
        }
    }
        ...
        if (!$hasSession) {
            ...
            if ($session) {
                ...
                if ($email == '') {
                    ...
                    $this->customerAction($firstName, $lastName, $email,$gender, 'Yahoo', $data);
                }
            }
        }
    }
