...
class Magestore_Shopbybrand_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function refineUrlKey($urlKey) {
        ...
        //removed htmlspecialchars as it was converint & sign to &amp @RT
    ...
    public function getDesignerImagePath($brandId) {
        //$brandImagePath = Mage::getBaseDir('media') . DS .'brands' .DS. strtolower(substr($brandName,0,1)).substr(md5($brandName),0,10). $this->refineUrlKey($brandName);
        $brandImagePath = Mage::getBaseDir('media') . DS . 'brands' . DS . 'designer' . DS . $brandId;
        return $brandImagePath;
    }

    public function getDesignerImagePathCache($brandId) {
        //$brandImagePathCache = Mage::getBaseDir('media') . DS .'brands' . DS .'cache'. DS . strtolower(substr($brandName,0,1)). substr(md5($brandName),0,10). $this->refineUrlKey($brandName);    
        $brandImagePathCache = Mage::getBaseDir('media') . DS . 'brands' . DS . 'designer' . DS . 'cache' . DS . $brandId;
        return $brandImagePathCache;
    }
    ...
    public function uploadIsDesignerImage($brandId, $imageFile) {
        $this->createDesignerImageFolder($brandId);

        $brandDesignerImagePath = $this->getDesignerImagePath($brandId);
        $brandDesignerImagePathCache = $this->getDesignerImagePathCache($brandId);
        ...
        if (isset($imageFile['name']) && $imageFile['name'] != '') {
            //try {
                /* Starting upload */
                $imageName = $imageFile['name'];
                $uploader = new Varien_File_Uploader('is_designer_image');
                $newImageName = $this->refineImageName($imageName);
                // Any extention would work
                $uploader->setAllowedExtensions(array('jpg', 'jpeg', 'gif', 'png'));
                $uploader->setAllowRenameFiles(true);
                ...
                $uploader->setFilesDispersion(false);
                $uploader->save($brandDesignerImagePath, $newImageName);
                $newImageName = $uploader->getUploadedFileName();
                $fileImg = new Varien_Image($brandDesignerImagePath . DS . $newImageName);
                $fileImg->keepAspectRatio(true);
                $fileImg->keepFrame(true);
                $fileImg->keepTransparency(true);
                $fileImg->constrainOnly(false);
                ...
                $fileImg->resize(60, 60);
                $fileImg->save($brandDesignerImagePathCache . DS . $newImageName, null);
    ...
    public function deleteDesignerImageFile($brandName, $image) {

        if (!$image) {
            return;
        }
        $brandImagePath = $this->getDesignerImagePath($brandName) . DS . $image;
        $brandImagePathCache = $this->getDesignerImagePathCache($brandName) . DS . $image;
    ...
    public function createDesignerImageFolder($brandName) {
        $brandPath = Mage::getBaseDir('media') . DS . 'brands';
        $brandPathCache = Mage::getBaseDir('media') . DS . 'brands' . DS . 'designer' . DS . 'cache';

        $brandImagePath = $this->getDesignerImagePath($brandName);
        $brandImagePathCache = $this->getDesignerImagePathCache($brandName);
    ...
    public function getUrlDesignerImagePath($brandId) {
        //$brandImagePathUrl = Mage::getBaseUrl('media') .'brands/cache/'. strtolower(substr($brandName,0,1)). substr(md5($brandName),0,10) . $this->refineUrlKey($brandName);  
        $brandImagePathUrl = Mage::getBaseUrl('media') . 'brands/designer/cache/' . $brandId;
        return $brandImagePathUrl;
    }
...
//                                )->where('stock.is_in_stock = 1');
    ...
    public function copyDesignerImage($fromName, $fromImage, $toName, $toImage) {
        $this->createDesignerImageFolder($toName);
        $newPath = $this->getDesignerImagePath($toName) . DS . $toImage;
        $oldPath = $this->getDesignerImagePath($fromName) . DS . $fromImage;
        $newPathCache = $this->getDesignerImagePathCache($toName) . DS . $toImage;
        $oldPathCache = $this->getDesignerImagePathCache($fromName) . DS . $fromImage;
        copy($oldPath, $newPath);
        copy($oldPathCache, $newPathCache);
    }
