        ...
        foreach ($data as $value) {
            ...
            if ($value['store_id'] == 0) {
                ...
                $dataBrand = array(
                    ...
                    'image' => $value['image'],
                    'is_designer_image' => $value['is_designer_image'],
                    'thumbnail_image' => $value['image_thumbnail'],
                    'is_featured' => $value['featured'],
                    'is_designer' => $value['designer'],
                    'is_upcoming' => $value['upcoming'],
                ...
                try {
                    ...
                    if($value['is_designer_image'])
                    $this->copyImagOldData($modelBrand,'is_designer_image',$value['is_designer_image']);
...
//                echo $exc->getTraceAsString();
                ...
                if (!$value['default_discount']) {
                    $this->saveStoreValue($modelBrand, $value['store_id'], 'percentage_discount', $value['discount']);
                }
                ...
                if (!$value['default_is_designer_image']  && $value['is_designer_image']) {
                    $this->saveStoreValue($modelBrand, $value['store_id'], 'is_designer_image', $value['is_designer_image']);
                }
                ...
                if (!$value['default_designer']) {
                    $this->saveStoreValue($modelBrand, $value['store_id'], 'is_designer', $value['designer']);
                }

                if (!$value['default_upcoming']) {
                    $this->saveStoreValue($modelBrand, $value['store_id'], 'is_upcoming', $value['upcoming']);
                }
                ...
                /*Information tab data save on store level*/
                if (!$value['is_show_information_tab_default']) {
                        $this->saveStoreValue($modelBrand, $value['store_id'], 'is_show_information_tab', $value['is_show_information_tab_default']);
                }
                if (!$value['information_title_default']) {
                        $this->saveStoreValue($modelBrand, $value['store_id'], 'information_title', $value['information_title_default']);
                }
                if (!$value['information_description_default']) {
                        $this->saveStoreValue($modelBrand, $value['store_id'], 'information_description', $value['information_description_default']);
                }
            }
        }
    }
    ...
    public function getPercentageDiscount($percentagediscount) {
        $collection = Mage::getModel('shopbybrand/brand')->getCollection()
                ->addFieldToFilter('percentage_discount', $percentagediscount);
        if ($collection->getSize()) {
            return $collection->getFirstItem();
        }
        return Mage::getModel('shopbybrand/brand');
        ...
        if($is_update=='3'){
            ...
            try {
                foreach ($csvData as $k => $v) {
                    ...
                    $storeId = Mage::getModel('core/store')->load($v[1], 'code')->getId();
                    $brand = Mage::getModel('shopbybrand/brand')->getCollection()
                        ->addFieldToFilter('brand_id',$v[0])
                        ->getFirstItem();
                    if($brand->getId()){
                        $brand->setPageTitle($v[2])
                            ->setMetaDescription($v[3])
                            ->setName($v[4])
                            ->setStoreId($v[1])
                            ->save();
                        $number['update']++;
