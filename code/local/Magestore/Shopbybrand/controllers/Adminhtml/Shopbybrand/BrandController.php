...
//        $this->renderLayout();
    ...
    public function saveAction() {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if (isset($data['is_designer_image']['delete'])) {
                Mage::helper('shopbybrand')->deleteDesignerImageFile($data['name'], $data['old_is_designer_image']);
                unset($data['old_is_designer_image']);
            }
            $data['is_designer_image'] = "";
            if (isset($_FILES['is_designer_image']))
                $data['is_designer_image'] = Mage::helper('shopbybrand')->refineImageName($_FILES['is_designer_image']['name']);

            if (!$data['is_designer_image'] && isset($data['old_is_designer_image'])) {
                $data['is_designer_image'] = $data['old_is_designer_image'];
            }
            ...
            try {
                ...
                try {
                    $model->save();
                    $brandId = $model->getId();
                    $this->brandStoreValue($brandId,$store, $data) ;
                ...
                $isdesignerimage = $model->getIsDesignerImage();
                if (isset($_FILES['is_designer_image'])) {
                    if (isset($_FILES['is_designer_image']['name']) && $_FILES['is_designer_image']['name'])
                        $isdesignerimage = Mage::helper('shopbybrand')->uploadIsDesignerImage($model->getId(), $_FILES['is_designer_image']);
                }
                ...
                if ($image != $model->getImage() || $thumbnailImage != $model->getThumbnailImage() || $isdesignerimage != $model->getIsDesignerImage()) {
                    if ($image != $model->getImage())
                        $model->setImage($image);
                    if ($thumbnailImage != $model->getThumbnailImage())
                        $model->setThumbnailImage($thumbnailImage);
                    if ($isdesignerimage != $model->getIsDesignerImage())
                        $model->setImage($isdesignerimage);
                    $model->save();
                }
    ...
    /* Newly added attribute added into the store table , So it can work for individual store.*/
    public function brandStoreValue( $brandId, $store, $data ){
        $attributes = array('information_description','information_title','is_show_information_tab');
        $attributeValue = Mage::getModel('shopbybrand/brandvalue');//Load Brand Value Model
        foreach( $attributes as $attribute ){
            if (!$data[$attribute.'_default']) {
                $attributeValueData = $attributeValue->loadAttributeValue( $brandId , $store, $attribute );
                try {
                    $attributeValue->setValue($data[$attribute])->save();
                }catch (Exception $e) {
