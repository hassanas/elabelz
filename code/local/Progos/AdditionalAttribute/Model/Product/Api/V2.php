 ...
 *
 * @category   Progos
 * @package    Progos_AdditionalAttribute
 * @author     Saroop
 * @date       15-05-2017
 */
class Progos_AdditionalAttribute_Model_Product_Api_V2 extends Bubble_Api_Model_Catalog_Product_Api_V2
{


    /*
    * Core Method changed because ADDING ADDITIONAL ATTRIBUTES IN MAGENTO V2 API IN WS-I  
    * COMPLIANCE MODE did not worked when the attribute type is array.
    * Line No which is modified : 65-66
    */
   ...
   public function create($type, $set, $sku, $productData, $store = null)
    ...
    protected function _prepareDataForSave($product, $productData)
    {
        ...
        if (property_exists($productData, 'additional_attributes')) {
            ...
            if ($singleDataExists || $multiDataExists || gettype($productData->additional_attributes) == 'array' ) {
                if ($singleDataExists) {
                ...
                if (gettype($productData->additional_attributes) == 'array') {
                    foreach ($productData->additional_attributes as $k => $v) {
                        $_attrCode = $k;
                        $productData->$_attrCode = Mage::helper('bubble_api/catalog_product')
                            ->getOptionKeyByLabel($_attrCode, $v);
