 ...
 */
class Progos_ShopBy_Controller_Router extends Amasty_Shopby_Controller_Router
{
    const MIDDLE    = 0;
    const BEGINNING = 1;
    ...
    public function match(Zend_Controller_Request_Http $request)
    {
        ...
        if (!$isMatched && $this->urlMode == Progos_ShopBy_Model_Shopby_Source_Url_Mode::MODE_CUSTOM) {
            $isMatched = $this->matchForwardCustom();
        }
    ...
    protected function matchForwardCustom()
    {
        $pageId = $this->request->getPathInfo();
        $pageId = $this->getUrlHelper()->checkRemoveSuffix($pageId);
        $pageId = trim($pageId, '/');
