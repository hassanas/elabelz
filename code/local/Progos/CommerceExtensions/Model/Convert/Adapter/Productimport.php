 ...
 * @Date: 15-05-2017
 * This file extended to add SEO friendly URLS as per the following template at the time of product import
 * buy-BRAND-PRODUCT-NAME-for-MAIN-CATEGORY-LAST-CATEGORY-PRODUCT-ID
 * WORK AT LINE# 188
 *
 * */

class Progos_CommerceExtensions_Model_Convert_Adapter_Productimport extends CommerceExtensions_Productimportexport_Model_Convert_Adapter_Productimport
{
    ...
    public function saveRow(array $importData)
    {
        ...
        if ($allowUpdateOnly == false) {
            ...
            if ($importData['prodtype'] == 'configurable') {
                ...
                // Hassan: set custom seo url code start
                if ($importData['store'] == 'admin') {
                    $seoUrl = "buy ";
                    if (!empty($importData['manufacturer'])) {
                        $seoUrl .= $importData['manufacturer'] . " ";
                    }
                    $seoUrl .= $importData['name'] . " ";
                    if (!empty($importData['category_ids'])) {
                        $cats = explode(',', $importData['category_ids']);
                        sort($cats);
                ...
                $product->setUrlKey(Mage::getSingleton('core/session')->getSeoProductUrl());
            }
            // Hassan: set custom seo url code end
...
#}
    ...
    /**
     * Hassan
     * This is private function so need to import here
     *
