 ...
 */
require Mage::getModuleDir('controllers', 'Infomodus_Dhllabel') . DS . 'Adminhtml/Dhllabel/DhllabelController.php';
class Progos_Dhl_Adminhtml_Dhllabel_DhllabelController extends Infomodus_Dhllabel_Adminhtml_Dhllabel_DhllabelController
{
    ...
    public $customerPaperless;

    public function __construct($opOne = null, $opTwo = null, $opThree = array())
    {
        $this->customerPaperless = $this->getCustomPaperLessSetting();
        if ($opOne != null) {
    ...
    public function getCustomPaperLessSetting(){
        $customPaperLessStatus = Mage::getStoreConfig('dhllabel/customerpaperless/enable');
        if( $customPaperLessStatus == '1' )
            $customPaperLessStatus = true;
        else
            $customPaperLessStatus = false;
        return $customPaperLessStatus;
    }
    ...
    public function saveDB($upsl, $upslTwo = null, $params, $orderId, $shipmentId, $type, $lbl = null)
    {
        ...
        $customerPaperStatus = 0;
        if( $this->customerPaperless ){
            if( $params['shiptocountrycode'] == "SA" ){
                $customerPaperStatus = 1;
            }else{
                $customerPaperStatus = Mage::getStoreConfig('dhllabel/paperless/create_pdf', $storeId);
            }
        }else{
            $customerPaperStatus = Mage::getStoreConfig('dhllabel/paperless/create_pdf', $storeId);
        }
    ...
    public function setParams($lbl, $params, $packages /*multistore*/, $storeId = null /*multistore*/, $order)
    {
        ...
        if( $this->customerPaperless ){
            if( $params['shiptocountrycode'] == "SA" ){
                ...
                $customerPaperStatus = Mage::getStoreConfig('dhllabel/printing/archive', $storeId);
            }
        }else{
            $customerPaperStatus = Mage::getStoreConfig('dhllabel/printing/archive', $storeId);
        }
    ...
    public function intermediatehandy($orderId, $type, $shipmentId = null, $shipperAddress = null)
    {
        ...
        foreach ($shipmentAllItems as $item) {
            ...
            if (!$item->isDeleted() && !$item->getParentItemId()) {
                ...
                $itemData['qty'] = $itemData['qty'] - $itemData['qty_canceled'];
                ...
                //Convert Item Price to 40% #One Line added By Saroop
                $itemPrice = ( $itemPrice * 40 ) / 100;
