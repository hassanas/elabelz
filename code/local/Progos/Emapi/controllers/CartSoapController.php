<?php
/**
 * This Controller is created to handle the cart functionality performed from mobile apps
 * @category     Progos
...
class Progos_Emapi_CartSoapController extends Progos_Emapi_SoapController
{
    ...
    public function getCartAction()
    {
        ...
        if (!$quoteId) {
            ...
            try {
                ...
                $cartData = $this->info($quoteId,$isfloat);
     ...
     *
     */
    public function info($quoteId,$isfloat=null)
    {
        for ($i = 0; $i < 3; $i++) {
        ...
        $i = 0;
        if($isfloat){
            $data['grand_total'] = (float)number_format((float)$data['grand_total'], 2,'.','');
            $data['base_grand_total'] = (float)number_format((float)$data['base_grand_total'], 2,'.','');
            $data['subtotal'] = (float)number_format((float)$data['subtotal'], 2,'.','');
            $data['base_subtotal'] = (float)number_format((float)$data['base_subtotal'], 2,'.','');
            $data['subtotal_with_discount'] = (float)number_format((float)$data['subtotal_with_discount'], 2,'.','');
            $data['base_subtotal_with_discount'] = (float)number_format((float)$data['base_subtotal_with_discount'], 2,'.','');
        }else{
        ...
        foreach ($data['items'] as $item) {
            ...
            if ($item['price'] == 0) {
                ...
                if($isfloat){
                    $data['items'][$i]['sale_price'] = 0;
                    $data['items'][$i]['actual_price'] = (float)number_format((float)Mage::helper('core')->currency($obj->getPrice(), false, false), 2,'.','');
                    $data['items'][$i]['final_price'] = (float)number_format((float)Mage::helper('core')->currency($obj->getFinalPrice(), false, false), 2,'.','');
                    if($data['items'][$i]['actual_price'] != $data['items'][$i]['final_price']){
                        $data['items'][$i]['sale_price'] = (float)number_format((float)Mage::helper('core')->currency($obj->getFinalPrice(), false, false), 2,'.','');
                    }
                    $data['items'][$i]['price'] = (float)number_format((float)$price, 2,'.','');
                    $data['items'][$i]['row_total'] = (float)number_format((float)$rowTotal, 2,'.','');
                }else{
            ...
            if ($cdnQueryString != "") {
                $data['items'][$i]['img'] = $image. '?query=' . $cdnQueryString;;
                $data['items'][$i]['img2'] = $product->getImageUrl(). '?query=' . $cdnQueryString;;
            }
        ...
        if (intval($data['subtotal']) == 0) {
            if($isfloat){
                $data['subtotal'] = (float)number_format((float)$subtotal, 2,'.','');
                $data['subtotal_with_discount'] = (float)number_format((float)$subtotal, 2,'.','');
            }else{
     ...
     *
     */
    public function getPaymentShipmentList($quoteId,$isfloat=null)
    {
        ...
        try {
            ...
            foreach ($result->complexObjectArray as $r) {
                if ($r->code != 'telrpayments_cc') {
                    ...
                    if ($r->code == 'msp_cashondelivery') {
                        ...
                        if($isfloat){
                            $payment[$p]['price'] = (float)number_format((float)Mage::helper('core')->currency($amount, false, false), 2,'.','');
                        }else{
            ...
            foreach ($resultshipping->complexObjectArray as $r) {
                ...
                if($isfloat){
                    $shipping[$p]['price'] = (float)number_format((float)Mage::helper('core')->currency($r->price, false, false), 2,'.','');
                }else{
    ...
    public function getPaymentShipmentListSoapless($shippingCountry, $billingCountry, $orderSubtotal, $isfloat)
    {
        if (Mage::getStoreConfig('api/emapi/enableCachedPaymentShipment')) {
            try {
                $storeId = $store = Mage::app()->getStore()->getId();
                $payment = $this->getPaymentMethods($billingCountry, $isfloat);
                ...
                if ($isfloat) {
                    $shipping[0]['price'] = (float)number_format((float)Mage::helper('core')->currency($price, false, false), 2, '.', '');
                } else {
            ...
            try {
                ...
                if($codEnabled) {
                    ...
                    if ($isfloat) {
                        $payment[0]['price'] = (float)number_format((float)Mage::helper('core')->currency($amount, false, false), 2, '.', '');
                    } else {
    ...
    private function getPaymentMethods($billingCountry,$isfloat){
        /*
        ...
        if($codEnabled) {
            ...
            if ($isfloat) {
                $cod['price'] = (float)number_format((float)Mage::helper('core')->currency($amount, false, false), 2, '.', '');
            } else {
        ...
        if (Mage::getStoreConfig('api/emapi/setShippingAddress')) {
            try {
                ...
                if($mdlEmapi->getShippingCustomerInfo()) {
                    $quote = Mage::getModel("sales/quote")->loadByIdWithoutStore($quoteId);
                    $paymentShipment = $this->getPaymentShipmentListSoapless($address['country_id'], $address['country_id'], $quote->getBaseSubtotal(), $isfloat);
                    if ($paymentShipment['status']) {
                        ...
                        if($quote->getBaseSubtotalWithDiscount() == 0){
                            $responseDiscounted = $this->getDiscountedPaymentShipment($address['country_id']);
                        ...
                        $storeCredit[0]['price'] = 0;
                        if(isset($isUserLoggedIn) && $isUserLoggedIn == 1){
                            $storeCredit = Mage::helper('emapi')->getStoreCredits($customer->getId(), $isfloat);
                        }elseif(!isset($isUserLoggedIn)) {
                            $storeCredit = Mage::helper('emapi')->getStoreCredits($customer->getId(), $isfloat);
                        }
            ...
            try {
                ...
                if (($qsa->getFirstname() == null || $qsa->getLastname() == null
                    ...
                    if (Mage::getStoreConfig('api/emapi/getPaymentShipment')) {
                        ...
                        $paymentShipment = $this->getPaymentShipmentList($quoteId,$isfloat);
                    }
                    if ($mode == "shipping") {
                        $response['message'] = 'Shipping address added successfully';
                    } else {
    ...
    /*
     * Function to get payment and shipment methods in case of 100% discount
     */
    function getDiscountedPaymentShipment($country){
        $response = array();
        $discountedShipping = array();
        if ($country == "AE") {
            $method = "freeshipping_freeshipping";
        } else {
            $method = "tablerate_bestway";
        }
        $discountedShipping[0]['code'] = $method;
        $discountedShipping[0]['title'] = "&nbsp";
        $discountedShipping[0]['price'] = 0;

        $discountedPayments = array();
        $discountedPayments[0]['code'] = "free";
        $discountedPayments[0]['title'] = __("No Payment Information Required");
        $discountedPayments[0]['price'] = 0;

        $response['shipping'] = $discountedShipping;
        $response['payment'] = $discountedPayments;
    }
}
