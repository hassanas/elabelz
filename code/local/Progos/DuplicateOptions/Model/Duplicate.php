...
class Progos_DuplicateOptions_Model_Duplicate extends Mage_Core_Model_Abstract
{	
	protected $_resource;
	...
	function __construct()
	{
		...
		$dryRun = Mage::getStoreConfig('duplicateoptions/duplicateoptions/dryrun_enable');
	...
	{
		$attributes = Mage::helper('duplicateoptions')->getOptionsfromConfig();
	...
	public function getCsvToReplaceColors()
	{
		$fileName = Mage::helper('duplicateoptions')->getOptionReplaceFile();
		...
		if (($handle = fopen($fileName, "r")) !== FALSE) {
		  	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
		    	$dbColorName = $data[0];
		    	$replaceWithColor = $data[1];

			    if($dbColorName && $replaceWithColor) {
			    	Mage::log("Color Name in Database: ".$dbColorName, null, "csv_duplicate_colors_".$this->_logFile);
					Mage::log("Replace with Color Name:".$replaceWithColor, null, "csv_duplicate_colors_".$this->_logFile);

					$query = "UPDATE $tableName SET value = '$replaceWithColor' WHERE value = '$dbColorName'";

					if($this->_isDryRun == 0) {
						$this->_writeConnection->query($query);
					}

					Mage::log("Update Query: ".$query.PHP_EOL.PHP_EOL, null, "csv_duplicate_colors_".$this->_logFile);
			    }
		  	}
		  fclose($handle);
		}
