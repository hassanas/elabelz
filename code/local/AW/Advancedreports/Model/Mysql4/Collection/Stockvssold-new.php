...
class AW_Advancedreports_Model_Mysql4_Collection_Stockvssold extends AW_Advancedreports_Model_Mysql4_Collection_Abstract
{
    ...
    {
        $orderTable = Mage::helper('advancedreports/sql')->getTable('sales_flat_order');
        $productTable = Mage::helper('advancedreports/sql')->getTable('catalog_product_super_link');
        ...
        echo $this->getSelect()
            ...
            ->joinLeft(
                ...
                "(main_table.entity_id = item2.order_id AND item.parent_item_id IS NOT NULL AND product.parent_id = item2.product_id AND item2.product_type = 'configurable')",
                array()
                )
            ->joinLeft(
                array('product' => $productTable),
                "(product.parent_id = item2.product_id )",
                array(
                    'configurable_product' => 'GROUP_CONCAT(product.product_id)',
                    'parent_item_id' =>'item2.product_id' )
            )
            ->joinLeft(
