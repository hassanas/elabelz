...
class AW_Storecredit_Model_Observer_Totals extends Mage_Core_Model_Abstract
{
    ...
    public function salesOrderInvoiceLoadAfter(Varien_Event_Observer $observer)
    {
        ...
        if($order->getCustomerIsGuest()): // add the check for guest customer
             return $this;
        endif;
         $customer=Mage::getModel('aw_storecredit/storecredit')->loadByCustomerId($customerId);
        if(empty($customer->getData())):  // add the check for customer have record in store credit
            return $this;
        endif;
