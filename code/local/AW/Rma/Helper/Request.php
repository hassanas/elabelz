        ...
        if ($_data['order_id']) {
            ...
            if (!($_order->getData() == array())) {
                ...
                if ($guestMode) {
                    ...
                    if (($_guestOrderId) && ($_orderIncrementId != $_order->getIncrementId())) {
                        ...
                        self::_getSession()->addError(
                            Mage::helper('awrma')->__('You are not authorized to Return Request for specified order')
                        );
                    }
                } else {
                ...
                if (!Mage::helper('awrma')->isAllowedForOrder($_order)) {
                    $_addNewEntityFlag = false;
                    self::_getSession()->addError(
                        Mage::helper('awrma')->__('You can\'t Return Request for the given order')
                    );
                }
                //Getting order items
                $_orderItems = array();
                        ...
                        if ($_addNewEntityFlag) {
                            ...
                            if (!($packageOpenedOption === false)) {
                                ...
                                self::_getSession()->addSuccess(
                                    Mage::helper('awrma')->__('New Return Request has been successfully added')
                                );
                                if (self::getIsNeedNotifyWaitingForApproval()) {
                                    self::_getSession()->addNotice(
                                        Mage::helper('awrma')->__('Your Return Request is currently waiting for approval')
                                    );
                                }
