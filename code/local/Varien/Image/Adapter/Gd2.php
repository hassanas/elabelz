...
class Varien_Image_Adapter_Gd2 extends Varien_Image_Adapter_Abstract
{
    ...
    {
        if ((string)Mage::getConfig()->getModuleConfig('Neklo_Imagick')->active != 'true' || !Mage::helper('neklo_imagick')->isEnabled()) {
            $this->origSave($destination, $newName);
        } else {
            ...
            // fix for imagick
            $baseFile = $this->_fileSrcPath . DS . $this->_fileSrcName;
            $imagick = new Imagick($baseFile);

            if ($this->_fileType == IMAGETYPE_JPEG) {
                $imagick->setImageFormat('jpeg');
            } elseif ($this->_fileType == IMAGETYPE_PNG) {
                $imagick->setImageFormat('png');
            } elseif ($this->_fileType == IMAGETYPE_GIF) {
                $imagick->setImageFormat('gif');
            }

            $helper = Mage::helper('neklo_imagick');
            $imagick->setImageCompressionQuality($helper->getQuality());

            $imagick->setBackgroundColor(new ImagickPixel('#FFFFFF'));

            if ($this->_imageSrcWidth && $this->_imageSrcHeight) {
                if ($helper->getResizeMode() == Neklo_Imagick_Helper_Data::RESIZE_MODE_THUMBNAIL) {
                    $imagick->thumbnailImage($this->_imageSrcWidth, $this->_imageSrcHeight, true, true);
                } elseif ($helper->getResizeMode() == Neklo_Imagick_Helper_Data::RESIZE_MODE_ADAPTIVE_RESIZE) {
                    $imagick->adaptiveResizeImage($this->_imageSrcWidth, $this->_imageSrcHeight, true);
                }
            }
    ...
    public function origSave($destination=null, $newName=null)
    {
    ...
    {
        if ((string)Mage::getConfig()->getModuleConfig('Neklo_Imagick')->active != 'true' || !Mage::helper('neklo_imagick')->isEnabled()) {
            $this->origResize($frameWidth, $frameHeight);
        } else {
    ...
    public function origResize($frameWidth = null, $frameHeight = null)
    {
        if (empty($frameWidth) && empty($frameHeight)) {
            throw new Exception('Invalid image dimensions.');
        }
