...
class Amasty_Xlanding_Adminhtml_LpageController extends Mage_Adminhtml_Controller_Action
{
    ...
    const FEATURED_LINKS = "featured_links";
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if ($id = $this->getRequest()->getParam('id')) {
                ...
                if ($this->getRequest()->getParam('duplicate')) {
                    $this->_redirect('adminhtml/lpage/edit', array('id' => $model->getPageId(), 'duplicate' => '1'));
                    return ;
                }
            }
            ...
            $link_pages = $this->getRequest()->getPost('link_pages');
            
           // echo '<pre>'; print_r($link_pages); echo '</pre>'; exit;
    ...
    public function prepareForSave($model)
    {
		...
		if (is_array($attributeValues)) {
			foreach ($attributeValues as $index => $value) {
                ...
                $featured_links = Mage::app()->getRequest()->getParam('featured_links');
                
                if(is_array($featured_links) && count($featured_links)>0 )
                $featured_links = array_filter($featured_links);
                
                if(is_array($featured_links) && count($featured_links)>0 ){
                    $validArray[self::FEATURED_LINKS] = Mage::app()->getRequest()->getParam('featured_links');
                }
    ...
    public function prepareForEdit($model)
    {
        ...
        if ($this->getRequest()->getParam('duplicate')) {
            $model->setId(null);
            $model->setTitle($model->getTitle() . '-duplicate');
            $model->setIdentifier($model->getIdentifier() . '-duplicate');
        }
