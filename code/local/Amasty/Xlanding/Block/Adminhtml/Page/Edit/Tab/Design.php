    ...
    implements Mage_Adminhtml_Block_Widget_Tab_Interface
{
    ...
    protected function _prepareForm()
    {
        ...
        */
        ...
        $fieldset->addField('is_hide_condition_filters', 'select', array(
            'name'     => 'is_hide_condition_filters',
            'label'    => $helper->__('Hide filters used for landing page creation.'),
            'values'   => Mage::getSingleton('adminhtml/system_config_source_yesno')->toArray(),
            'note'     => $helper->__('Only for Amasty Improved Layered Navigation')
        ));
        ...
        $mapper
            ->addFieldMap('page_include_navigation', 'page_include_navigation')
            ->addFieldMap('page_is_hide_condition_filters', 'page_is_hide_condition_filters')
            ->addFieldDependence('page_is_hide_condition_filters', 'page_include_navigation',
                array(Amasty_Xlanding_Model_Source_Navigation::INCLUDE_LEFT, Amasty_Xlanding_Model_Source_Navigation::INCLUDE_RIGHT))
        ;
        ...
        $layoutFieldset->addField('custom_design', 'select', array(
            'name'     => 'custom_design',
            'label'    => Mage::helper('core')->__('Custom Design'),
            'values'   => Mage::getModel('core/design_source_design')->getAllOptions(true),
        ));

        if (!$model->getId() && !$this->getRequest()->getParam('duplicate')) {
