...
class Amasty_Shopby_Block_Catalog_Layer_View extends Amasty_Shopby_Block_Catalog_Layer_View_Adapter
{
    ...
    /**
    *   Added by mbt 2018-02-08
    *   return empty layered nav on below specific lander pages 
    */
    protected function _toHtml() {

        if(Mage::registry('current_category')){
            $c_cat = Mage::registry('current_category');
            $c_catName = strtolower($c_cat->getName());
            $currentUrl = Mage::helper('core/url')->getCurrentUrl();
            $refinedString = str_replace("/", "", substr($currentUrl, strpos($currentUrl, $c_catName)));
            if ($c_cat->getId()==3 && $c_catName =='women') {
                if (strlen($refinedString) < 6) {
                    return '';
                }
            } else if ($c_cat->getId()==4 && $c_catName =='men') {
                if (strlen($refinedString) < 4) {
                    return '';
                }
            } else if ($c_cat->getId()==5 && $c_catName =='kids') {
                if (strlen($refinedString) < 5) {
                    return '';
                }
            }
        }
    ...
    protected function _prepareLayout()
    {
        ...
        if ($head){
            ...
            if (Mage::getStoreConfigFlag('amshopby/block/ajax')){
                ...
                $isMobile = $themeName == 'mobile';

                if (!$isProductPage && $isMobile) {
                    $head->addJs('amasty/amshopby/amshopby-mobile-ajax.js');
                } elseif(!$isProductPage) {
                    $head->addItem('skin_js','amasty/amshopby/amshopby-ajax.js');                 
                }
            }
        }
