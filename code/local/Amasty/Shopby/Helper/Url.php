...
class Amasty_Shopby_Helper_Url extends Mage_Core_Helper_Abstract
{
    ...
    protected function _getCurrentUrlWithoutParams()
    {
        ...
        if(array_key_exists('size', $query)):     
           ...
           if (array_key_exists('size', $hash)) {
            $dataHelper = Mage::helper('amshopby');
            $values = $dataHelper->getRequestValues('size');
                if (!$values) {
                    continue;
                }
                $value = $values[0];
                $foundOptionAlias = array_search($value, $hash['size']);
           }
           $url_size  = explode("/",$url_new[0]);
           $url_all = end($url_size);
           if($url_all == $foundOptionAlias ):
             array_pop($url_size);
             $url_new[0] = implode("/",$url_size);
            endif;
        endif;

        // if(strpos($url_new[0],$foundOptionAlias)):
        //     $url_new[0] = str_replace($foundOptionAlias, '', $url_new[0]);
        // endif;
        // echo $url_new[0];
        return $url_new[0];
    }
    ...
    public function getCurrentUrl()
    {
     ...
     public function getUrlPrice($url){
        $url_new = $url;
        $pos = max(0, strpos($url, '?'));
        if ($pos) {
            $url = substr($url, 0, $pos);
