...
class Fetchr_Shipping_Model_Observer{
    ...
    public function pushOrderAfterShipmentCreation($observer)
    {
        ...
        #12-03-2016 (Azhar) update date of shipped order items when order shipped scussesfully
    ...
    public function addFetcherButton($observer){
        $container = $observer->getBlock();
        if(null !== $container && $container->getType() == 'adminhtml/sales_order_view') {
            $ManualCODPush    = Mage::getStoreConfig('carriers/fetchr/manualcodpush');
            
            $order_id = Mage::app()->getRequest()->getParam('order_id');
            $order = Mage::getModel('sales/order')->load($order_id );
            $activeShippingCountry  = Mage::getStoreConfig('carriers/fetchr/specificcountry');
            $activeShippingCountry  = explode(',', $activeShippingCountry);
            $_shippingAddress = $order->getShippingAddress()->getCountry_id();

            if($ManualCODPush == true && $order->canShip() && in_array($_shippingAddress, $activeShippingCountry) ):  
            $data = array(
                'label'     => 'Fetcher Print Label',
                'class'     => 'go',
                'onclick'   => 'setLocation(\' '  . Mage::getUrl('fetchr/index/createtrackingno/', array('order_id' => $order_id)) . '\')',
            );
            $container->addButton('fetchr_print_label', $data);
            endif;
        }
        return $this;
