...
class Mirasvit_FeedExport_Model_Rule_Condition_Product extends Mage_Rule_Model_Condition_Abstract
{
    ...
    protected function _addSpecialAttributes(array &$attributes)
    {
        $attributes = array_merge($attributes, array(
            ...
            'is_all_in_stock'  => __('Stock Availability (Advanced)')
        ));
    }
    ...
    protected function _prepareValueOptions()
    {
        ...
        if ($this->getAttribute() === 'attribute_set_id') {
            ...
            $selectOptions = Mage::getResourceModel('eav/entity_attribute_set_collection')
                ...
                ->load()
                ->toOptionArray();
        } elseif ($this->getAttribute() === 'is_in_stock' || $this->getAttribute() === 'is_all_in_stock') {
            $selectOptions = array();
    ...
    public function collectValidatedAttributes($productCollection)
    {
        ...
        if (!in_array($attribute, array('category_ids', 'qty', 'php', 'is_in_stock', 'manage_stock', 'status_parent', 'is_salable', 'is_all_in_stock'))) {
    ...
    public function getInputType()
    {
        if ($this->getAttribute() === 'attribute_set_id' ||
            ...
            $this->getAttribute() === 'is_all_in_stock'
        ) {
            return 'select';
        }
    ...
    public function validate(Varien_Object $object)
    {
        ...
        switch ($attrCode) {
            ...
            case 'is_all_in_stock':
                $stockItem = Mage::getModel('cataloginventory/stock_item')->loadByProduct($object->getId());
                $isInStock = $stockItem->getIsInStock();
                $type = $object->getTypeId();

                if (($type == 'bundle' || $type == 'configurable') && $isInStock) {
                    $products = array();
                    $isInStock = 0;

                    if ($type == 'bundle') {
                        $selectionCollection = $object->getTypeInstance(true)->getSelectionsCollection(
                            $object->getTypeInstance(true)->getOptionsIds($object), $object
                        );
                        $productIds = array();
                        ...
                        $instance = $object->getTypeInstance();
                        $products = $instance->getUsedProducts(null, $object);
                    }

                    foreach ($products as $product) {
                        if (Mage::getModel('cataloginventory/stock_item')->loadByProduct($product->getId())->getIsInStock()) {
                            $isInStock = 1;
                        }
                    }
                }
                $result =  $this->validateAttribute($isInStock);
