        ...
        if (!$this->hasAllowProducts()) {
            ...
            foreach ($allProducts as $product) {
               $stockItem = Mage::getModel('cataloginventory/stock_item');
                $resource = $stockItem->loadByProduct($product->getId());
                if (($product->isSaleable() || $skipSaleableCheck) && (round($resource->getQty(),0) > 0)) {
                    if( $this->getRequest()->getRouteName().DS. $this->getRequest()->getControllerName().DS.$this->getRequest()->getActionName() == 'adminhtml/sales_order_create/configureProductToAdd' ){
                        if( $resource->getIsInStock() )
                         $products[] = $product;
                    }else{
                        $products[] = $product;
                    }
