...
class Mage_Checkout_OnepageController extends Mage_Checkout_Controller_Action
{
	...
	public function checkoutCouponPostAction () { 
		
		$_getCart = Mage::getSingleton('checkout/cart');
		$_getQuote = $_getCart->getQuote();
		
		/**
         * No reason continue with empty shopping cart
         */
        if (!$_getCart->getQuote()->getItemsCount()) {
            $response = array('success'=> false, 'error'=>'invalid_access', 'message'=>$this->__('Your shopping cart is empty.'));
        ...
        $oldCouponCode = $_getQuote->getCouponCode();
//var_dump($_getQuote);
        if (!strlen($couponCode) && !strlen($oldCouponCode)) {
            $response = array('success'=> false, 'error'=>true, 'message'=> $this->__('Provide coupon code!'));
			echo json_encode($response);die;
        }
        ...
        try {
            ...
            $_getQuote->getShippingAddress()->setCollectShippingRates(true);
            $_getQuote->setCouponCode($isCodeLengthValid ? $couponCode : '')
                ->collectTotals()
                ->save();
            //var_dump($_getQuote);exit;
            if ($codeLength) {
                if ($isCodeLengthValid && $couponCode == $_getQuote->getCouponCode()) {
					$response = array('success'=> true, 'error'=>false, 'message'=> $this->__("Coupon code '%s' was applied.", Mage::helper('core')->escapeHtml($couponCode)));
                } else {
					$response = array('success'=> false, 'error'=>true, 'message'=> $this->__("Coupon code '%s' is not valid.", Mage::helper('core')->escapeHtml($couponCode)));
                }
            } else {
				$response = array('success'=> true, 'error'=>'', 'message'=> $this->__("Coupon code was canceled."));
            }

        } catch (Mage_Core_Exception $e) {
			$response = array('success'=> false, 'error'=>true, 'message'=> $e->getMessage());
        } catch (Exception $e) {
            Mage::logException($e);
			$response = array('success'=> false, 'error'=>true, 'message'=> $this->__("Cannot apply the coupon code."));
        }
		// set identifier for showing only coupon code messages on cart page
		Mage::getSingleton('core/session')->setMyValue('coupon_code');
