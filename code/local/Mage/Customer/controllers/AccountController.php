...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    public function loginPostAction()
    {
       ...
       // $storeCode = Mage::helper("marketplace")->switchCustomerStore($loginChk["username"]);
        // if($storeCode !== false) {//means store switched
        //     $storeName = Mage::getModel('core/store')->load($storeCode, 'code')->getName();
        //     $msg = "You have registered on our $storeName store. Please login from our $storeName store.";
        //     $this->_redirectUrl("/".$storeCode."/customer/account/login/?msg=".urlencode($this->__($msg)));
        //     return;
        // }
                ...
                try {
                    ...
                    switch ($e->getCode()) {
                        case Mage_Customer_Model_Customer::EXCEPTION_EMAIL_NOT_CONFIRMED:
                            ...
                            $message = $this->_getHelper('customer')->__('This account is not confirmed.');
							$message .= $this->_getHelper('customer')->__('<a href="%s">', $value);
							$message .= $this->_getHelper('customer')->__('Click here');
							$message .= $this->_getHelper('customer')->__('</a>');
							$message .= $this->_getHelper('customer')->__(' to resend confirmation email.');
    ...
    protected function _successProcessRegistration(Mage_Customer_Model_Customer $customer)
    {
        $session = $this->_getSession();
        /*If session is not created for firsttime registration */
        if( ! Mage::getSingleton('checkout/session')->getRegisterCustomerFirstTime("recent") ){
                Mage::getSingleton('checkout/session')->setRegisterCustomerFirstTime("recent");
        }
    ...
    public function resetPasswordAction()
    {
        try {
            ...
            $this->_saveRestorePasswordParameters($customerId, $resetPasswordLinkToken)
                ->_redirect('*/*/changeforgotten',array('_query'=>array('___store'=>$backUrl)));

        } catch (Exception $exception) {
            $this->_getSession()->addError($this->_getHelper('customer')->__('Your password reset link has expired.'));
            $this->_redirect('*/*/forgotpassword',array('_query'=>array('___store'=>$backUrl)));
        }
    }
