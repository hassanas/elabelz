...
class Mage_Customer_Model_Customer extends Mage_Core_Model_Abstract
{
    ...
    public function sendNewAccountEmail($type = 'registered', $backUrl = '', $storeId = '0')
    {
        ...
        if (empty($backUrl) || is_null($backUrl)) {
        ...
        if($store->getCode() == "admin"):
           ...
           $r_store = "?___store=" . $store->getCode();
        else:
           $r_store = "?___store=" . $store->getCode();
        endif;
        
        $backUrl = Mage::getBaseUrl()."customer/account/" . $r_store;
    ...
    public function sendPasswordResetConfirmationEmail()
    {
        ...
        //$backUrls = Mage::getBaseUrl()."customer/account/changeforgotten/";
    ...
    public function validate()
    {
		...
		//echo Mage::getUrl('onestepcheckout');exit;
		Mage::log('Referrer URL:'.Mage::app()->getRequest()->getServer('HTTP_REFERER'));
		Mage::log('Checkout URL:'.Mage::getUrl('onestepcheckout'));
		
		/*if(Mage::app()->getRequest()->getServer('HTTP_REFERER') == Mage::getUrl('onestepcheckout')){
			$confirmation = $this->getConfirmation();
			Mage::log('Password mismatch: onestepcheckout');
			}else{*/
				$confirmation = $this->getPasswordConfirmation();
				//Mage::log('Password mismatch: other registeration stuff');
				//}
