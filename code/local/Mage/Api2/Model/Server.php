...
class Mage_Api2_Model_Server
{
    ...
    public function run()
    {
        ...
        try {
            ...
            if($this->checkSkipAuthenticationForRequest($request)) {
                $userModel = $this->getGuestUserModel();
                ...
                $apiUser = $userModel;
                Mage::register('api_rest_request_for_skip_authentication', true);
            } else {
     ...
     * @return boolean
     */
    public function checkSkipAuthenticationForRequest($request)
    {
        $apiPathToSkipForAuthentication = array('/api/rest/product/list', '/api/rest/product/detail', '/api/rest/product/filters', '/api/rest/search/filters', '/api/rest/product/search');
        $isSkipAuthentication = false;
        foreach ($apiPathToSkipForAuthentication as $path) {
            if(strpos($request->getPathInfo(), $path, 0) !== false) {
                 $isSkipAuthentication = true;
            }
        }
        return $isSkipAuthentication;
    }
    
    /**
     * 
     * @return Mage_Api2_Model_Auth_User_Guest
     */
    public function getGuestUserModel()
    {
        $helper    = Mage::helper('api2/data');
        $userTypes = $helper->getUserTypes();
        $userModel = Mage::getModel($userTypes[Mage_Api2_Model_Auth::DEFAULT_USER_TYPE]); 
